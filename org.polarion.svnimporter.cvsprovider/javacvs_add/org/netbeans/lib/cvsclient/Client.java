...
public class Client implements ClientServices, ResponseServices {
	...
	private boolean lsCommandNotSupported;
	...
	public void lsCommandNotSupported() {
		lsCommandNotSupported = true;
	}
	...
	public void processRequests(List requests)
		...
		for (Iterator it = requests.iterator(); it.hasNext();) {
			...
			if (request instanceof CommandRequest) {
				CommandRequest cr = (CommandRequest) request;
				if ("ls\n".equals(cr.getRequestString())) {
					if (lsCommandNotSupported) {
						throw new LsCommandNotSupported("The server is not supporting ls command");
					}
				}
			}
			//---[/add]-----------------------------------------------
